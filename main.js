...
// @exclude      *://bangumi.bilibili.com/anime/*/*/*
// @grant        GM_addStyle
// @grant        unsafeWindow
// ==/UserScript==
(function(e, t) {
...
function gTime() {
    ...
    var date = today.getFullYear().toString() + timenew(today.getMonth() + 1) + timenew(today.getDate()) + timenew(today.getHours()) + timenew(today.getMinutes());
...
console.log("已获取CSRF(请求令牌):", acsrf);(function() {
    ...
    $('#msgbutton').click(function msgbutton() {
        if (stopbutton === false) {
            var anime = $('#anime').val();
            var myep = $('#ep').val();
...
function updateText(responseText) {
    if (document.getElementById("infotxt") && responseText !== "") {
        var screenTxt = document.getElementById("infotxt");
        screenTxt.innerHTML = $('#infotxt').val() + "\n" + responseText;
...
function setmsg(anime, myep, mymessage) {
    ...
    function sendMessage(aid, message) {
        $.ajax({
            ...
            if (data.data && data.code === 0){
                ...
                updateText("正在快速重发");
                if(bilibilisendstop === false){
                    sendMessage(aid, message)}
                else{
                    updateText("发送已终止...");
        ...
        $.ajax({
            ...
            success:  function(newresult) {
                ...
                if (newresultindex != null && epnametest.test(newresult.data.feeds[newresultindex].addition.title) && epmyeptest.test(newresult.data.feeds[newresultindex].addition.description)){
                    console.log("当前更新EP:" + myep);
                    console.log("核心A: 已找到输入的EP:" + myep + ",AV:" + thisavid + "\n正在准备发送...");
                    updateText("核心A: 已找到输入的EP:" + myep + ",AV:" + thisavid + "\n正在准备发送...");
                    document.getElementById("msgbutton").value = "发送";
                    ...
                    //function, time, function parameter
                    console.log("核心A: 你要的是 EP:" + myep + ",尚未找到额...");
                    updateText("核心A: 你要的是 EP:" + myep + ",尚未找到额...");
                } else if (infoflushd === true) {
                    updateText("");
                } else {
                    updateText("核心A: 已停止发送...");
                    updateText("bilibili- ( ゜- ゜)つロ 乾杯~");
                    console.log("脚本已关闭...");
                }
            }
    ...
    function coreb() {
        $.getJSON("https://bangumi.bilibili.com/web_api/get_ep_list?season_id=" + anime,
                  function(newresult) {
            //api服务器目前响应头带no-cache,如果发生特殊情况,在.getJSON("https://bangumi.bilibili.com/web_api/get_ep_list?season_id="+anime后加上+"&_="+(new Date()).getTime()用时间戳刷掉缓存
            ...
            if (ep >= intmyep) {
                var id = eps[c].avid;
                console.log("核心B: 已找到输入的EP:" + myep + ",AV:" + id + "\n正在准备发送...");
                updateText("核心B: 已找到输入的EP:" + myep + ",AV:" + id + "\n正在准备发送...");
                document.getElementById("msgbutton").value = "发送";
                console.log(newresult);
                ...
                setTimeout(setmsg, 1, anime, myep, mymessage);
                console.log("核心B: 你要的是 EP:" + myep + ",尚未找到额...");
                updateText("核心B: 你要的是 EP:" + myep + ",尚未找到额...");
            } else if (infoflushd === true) {
                updateText("");
            } else {
                updateText("核心B: 已停止发送...");
                updateText("bilibili- ( ゜- ゜)つロ 乾杯~");
                console.log("脚本已关闭...");
            }
        });
    ...
    function automode(indata) {//不建议使用,只做功能性研究
        var data = gTime();
        var iTime = indata+"";
        if (data === iTime) {
            eptype.checked = false
            dulcorecheck = 0;
            document.getElementById('autest').innerHTML = "";
            corea();
        }else if(bilibilisendstop === false){
            document.getElementById('autest').innerHTML = "当前时间："+data+ " 状态：等待";
            setTimeout(automode, 500, iTime);
        }else{
            document.getElementById('autest').innerHTML = "";
            updateText("等待核心: 定时已终止...");
            updateText("bilibili- ( ゜- ゜)つロ 乾杯~");
            console.log("脚本已关闭...");
        }
    }
    ...
    if (eptype.checked === true){
        intime = prompt("请输入时间戳",gTime());
        updateText("将于'"+intime+"'启动发送");
        updateText("  ");
        automode(intime)
    }
